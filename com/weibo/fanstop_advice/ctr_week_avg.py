...
def ctr_week_avg(dir_path):
	...
	tmp_static = {'0fanstop_num':0,'0fanstop_interact':0.0,'1fanstop_num':0,'1fanstop_interact':0.0,'0feifen_num':0, \
	'0feifen_interact':0.0,'1feifen_num':0,'1feifen_interact':0.0,'0orientation_num':0,'0orientation_interact':0.0,'1orientation_num':0,'1orientation_interact':0.0}
	item_list = ['fanstop','feifen','orientation']
	expo_flag='_expo'
	expo_num =1
	sta_ctr = {}
	recmd_num = 0
	sta_num = {'0_fanstop_num':0,'1_fanstop_num':0,'0_feifen_num':0,'1_feifen_num':0,'0_orientation_num':0,'1_orientation_num':0}
	for (adid,values) in history_order.items():
		...
		for way in item_list:
			...
			tmp_static[str(values["video_flag"])+way+'_interact'] += float(values[way+'_ctr'])*values[way+expo_flag]
	...
	for way in item_list:
		...
		print '0_'+way+'_ctr_week:',tmp_static['0'+way+'_interact'],tmp_static['0'+way+'_num'],ctr_data['0_'+way+'_ctr_week']
		print '1_'+way+'_ctr_week:',tmp_static['1'+way+'_interact'],tmp_static['1'+way+'_num'],ctr_data['1_'+way+'_ctr_week']
	...
	with open(dir_path+'/data/ctr_week_avg.txt','r') as fr:
		for line in fr:
			...
			if float(ctr_week_out[take[0]])/float(take[1]) >1.25 or float(ctr_week_out[take[0]])/float(take[1]) < 0.75:
				update_flag = 0
	if 1 == update_flag:
