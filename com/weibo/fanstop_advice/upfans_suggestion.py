...
class Upfans_Suggestion(object):#涨粉
    ...
    def data_store(self):
        cPickle.dump(self.upfans_history_custom_record,
                     open(self.dir_path + '/data/upfans_history_custom_record.pkl', 'wb'))
        ...
        for (adid, values) in self.order_data.items():
            ...
            if ( 1 == buy_flag):
                if uid in self.upfans_history_custom_record:
                    ...
                    if uid not in self.upfans_recmd_record[upfans_mod].keys() or self.upfans_recmd_record[upfans_mod][
                        uid] < date_7:
