	...
	for (adid,value) in order_data.items():
		...
		try:
			try:
				...
				inter_data[adid] = [r_back.hgetall(adid+'_interact_advice_nums'),r_back.hgetall(adid+'_expo_user_profile')]
		...
		if adid in inter_data.keys():
			...
			if order_data[adid]['feifen_num'] != 0 :
				...
				if "extendfans" in inter_data[adid][1].keys():
					...
					order_data[adid]['feifen_ctr'] = float(ctr_num)/int(inter_data[adid][1]["extendfans"]) if int(inter_data[adid][1]["extendfans"]) !=0 else 0
			...
			if order_data[adid]['orientation_num'] != 0:
				...
				if "fieldfans" in inter_data[adid][1].keys():
					...
					order_data[adid]['orientation_ctr'] = float(ctr_num)/int(inter_data[adid][1]['fieldfans']) if int(inter_data[adid][1]['fieldfans']) !=0 else 0
			...
			if order_data[adid]['fanstop_num'] != 0:
				...
				if "fanstop" in inter_data[adid][1].keys():
					...
					order_data[adid]['fanstop_ctr'] = float(ctr_num)/int(inter_data[adid][1]['fanstop']) if int(inter_data[adid][1]['fanstop']) !=0 else 0
	...
	with open(output_path,'w') as fw:
		for (adid,v) in order_data.items():
			...
			if  adid in buy_result[adid_mod].keys():
				...
				fw_test.write('fentiao:'+tmp_result['suggestion']['fentiao']+'\t'+'effect:'+str(tmp_result['effect'])+'\t')
...
def main(dir_path):
	...
	print datetime.datetime.now()," load data finsh: order_data num is ",len(order_data),"history_order_data num is ",len(history_order_data)
	...
	print datetime.datetime.now()," ctr caculate finish: CtrData order_data num is ", len(order_data),"CtrData history_order_data num is ",len(history_order_data)
	...
	try:
		os.system(str1+"/example/TokenizerExampleCpp "+str1+"/conf/tokenizer.conf -e utf-8 -g both -w -f " + dir_path+"/data/mid_content.txt >"+dir_path+"/data/online_words.txt")
	except:
